...
Object.entries(mainnetArtifacts).forEach(([name, data]) => {
  if (data && data.address) {
    const info = extractContractInfo(data, name);
    if (info) {
      contractRegistry.mainnet[info.address] = info;
    }
  }
});

// Load Polygon contracts from mainnet.json (network 137)
if (mainnetArtifacts[NETWORKS.POLYGON]) {
  Object.entries(mainnetArtifacts[NETWORKS.POLYGON]).forEach(([name, data]) => {
    if (data && data.address) {
      const info = extractContractInfo(data, name);
      if (info) {
        contractRegistry.polygon[info.address] = info;
      }
    }
  });
}
...
// Load Tapir contracts
Object.entries(tapirArtifacts).forEach(([name, data]) => {
  if (data && data.address) {
    const info = extractContractInfo(data, name);
    if (info) {
      contractRegistry.tapir[info.address] = info;
    }
  }
});
...
export function getAccessControllerInfo(address, network = 'polygon') {
  ...
  if (name === 'GlobalAllowList') {
    return {
      ...
      type: 'global_allow_list',
      description: 'Allows all addresses to access the ritual',
      isProxy: address.toLowerCase() === '0x3e37c7a9a83b326a0d156de3ee6b18fd8079f698'
    };
